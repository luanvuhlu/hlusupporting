...
def logout_view(request):
    logout(request)
    return HttpResponseRedirect("/")
def check_login(request, usrn, pswd, remember):
    if not request.user.is_authenticated():
        return HttpResponseRedirect("/")
    return HttpResponse("Check login")
...
#                 day=day_week
                ...
                if day.week_day == day_count and day.start_hour==hour_count:
                    td['day']=day
                    td['slug']=slugify(day.timetable_subject.subject.subject.name + " "+day.timetable_subject.subject.student_year.name)
                    if day.type == False:
